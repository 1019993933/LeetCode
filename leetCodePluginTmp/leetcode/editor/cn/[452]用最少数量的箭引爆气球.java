...
class Solution {
    public int findMinArrowShots(int[][] points) {
        ...
        int count = 1;
        int commonLeft = points[0][0];
        int commonRight = points[0][1];
        for (int i = 1; i < points.length; i++) {
            if (points[i][0]>commonRight)
            {
                ++count;
                commonLeft = points[i][0];
                commonRight = points[i][1];
            }
            else
            {
                commonLeft = Math.max(commonLeft, points[i][0]);
                commonRight = Math.min(commonRight, points[i][1]);
            }
        }
        return count;
    }
